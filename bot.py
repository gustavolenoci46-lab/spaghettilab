...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.callback_query: await update.callback_query.edit_message_text(text=text, reply_markup=markup, parse_mode="Markdown")
    else: await update.message.reply_text(text=text, reply_markup=markup, parse_mode="Markdown")
...
async def manage_quantity_buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    parts = query.data.split("_"); action = parts[1]
    prod_id = f"{parts[2]}_{parts[3]}"; current_qty = int(parts[4])
...
async def add_to_cart_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    query = update.callback_query
    parts = query.data.split("_"); prod_id = f"{parts[1]}_{parts[2]}"; qty = int(parts[3])
...
async def show_cart(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query; await query.answer()
    cart = context.user_data.get('cart', {})
    if not cart: await query.edit_message_text("🛒 **Carrello vuoto!**", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Shop", callback_data="listings")] 없습니다]), parse_mode="Markdown"); return
